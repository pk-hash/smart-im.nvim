...
M.config = {
	...
	save_im_for_filetypes = {}, -- filetypes to track IM for (empty = all)
}
...
local function should_track_filetype(ft)
	...
	-- Empty config means track all filetypes
...
function M.restore_im(ft)
	...
	-- Use stored IM only if filetype is tracked
	if should_track_filetype(ft) then
		im = M._state.per_filetype[ft]
	end

	-- Fallback to default IM
